...
export class BulletBuzzGame {
  ...
  private axesThrown: number = 0;
  private enemiesSpawned: number = 0;
  private levelTimes: Record<number, string> = {};
  
  // Fixed timestep variables
  private accumulator: number = 0;
  private readonly timestep: number = 1/60; // 60 FPS fixed timestep
  ...
  // Shop system
  public shopOptions: ShopOption[] = [];
  private readonly upgradePool: ShopOption[] = [
  ...
  constructor(config: GameConfig = {}) {
    ...
    this.avoidDistance = config.avoidDistance ?? 80;
    this.avoidStrength = config.avoidStrength ?? 0.8;
    this.fleeRange = config.fleeRange ?? 25;
    ...
    this.levelScaling = config.levelScaling ?? 40;
    this.earlyLevelScaling = config.earlyLevelScaling ?? 6;
  ...
  private spawnEnemy(): void {
    ...
    const isWasp = this.level >= 3 && Math.random() < 0.3;
  ...
  private shootAxe(): void {
    ...
    for (let i = 0; i < this.projectileCount; i++) {
      ...
      // For random targets, use current position (no velocity)
      const predictedX = 'vx' in target ? target.x + ((target as any).vx || 0) * leadTime : target.x;
      const predictedY = 'vy' in target ? target.y + ((target as any).vy || 0) * leadTime : target.y;
      ...
      if (this.axes.length < this.maxAxes) {
        this.axes.push({
          x: this.player.x,
          y: this.player.y,
          vx: Math.cos(angle) * this.projectileSpeed * 2,
          vy: Math.sin(angle) * this.projectileSpeed * 2,
          rotation: 0,
          lifetime: 0
        });
