...
export class BulletBuzzGame {
    ...
    shootAxe() {
        if (this.enemies.length === 0)
            return;
        const inRangeEnemies = this.enemies.filter(e => Math.hypot(e.x - this.player.x, e.y - this.player.y) <= this.player.attackRange);
        ...
        for (let i = 0; i < this.projectileCount; i++) {
            ...
            const predictedX = 'vx' in target ? target.x + (target.vx || 0) * leadTime : target.x;
            const predictedY = 'vy' in target ? target.y + (target.vy || 0) * leadTime : target.y;
