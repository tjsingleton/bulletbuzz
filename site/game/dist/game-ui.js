...
function initGameUI() {
    ...
    const autoPathCheckbox = document.getElementById("autoPath");
    if (autoPathCheckbox) {
...
function draw() {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    game.game.heartDrops.forEach((h) => {
    ...
    // Draw XP bar
    const xpProgress = game.game.levelSystem.getXpProgress();
...
function drawGameOver() {
    ...
    const drawStatLine = (emoji, label, value, color) => {
        ctx.fillStyle = color;
        ctx.textAlign = 'left';
